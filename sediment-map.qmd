...
map_data = db.query(`
    ...
    WHERE s.param_id = '${selected_element.value}'
      AND CAST(STRFTIME('%Y', s1.sample_time) AS INTEGER) BETWEEN ${year_start} AND ${year_end}
    GROUP BY s.param_id, s2.lon, s2.lat
`)
  ...
  if (!container._map) {
    ...
    const HomeControl = L.Control.extend({
      ...
      onAdd(map) {
        ...
        // fa-expand icon: reset to default extent, widely understood on maps
...
point_data = {
  ...
  return db.query(`
    ...
    WHERE s.param_id = '${selected_element.value}'
     AND CAST(STRFTIME('%Y', s1.sample_time) AS INTEGER) BETWEEN ${year_start} AND ${year_end}
     AND s2.lon = ${p.lon} AND s2.lat = ${p.lat}
  `)
}
